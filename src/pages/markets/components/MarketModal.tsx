...
export const MarketModal = (props: {
  visible: boolean
  metaInfo?: {
    type: 'supply' | 'borrow'
    market?: Market
  ...
  const submitHandler = useCallback(async () => {
    ...
    if (actionType === 'Supply') {
      ...
      setUserBalance({
        ...
        borrowBalance: BigNumber(userBalance.borrowBalance).minus(amount).toString(),
      })
      ...
      setUserBalance({
        ...
        borrowBalance: BigNumber(userBalance.borrowBalance).plus(amount).toString(),
      })
    }
  ...
  const errorMessage = useMemo(() => {
    if (actionType === 'Supply' && BigNumber(amount || 0).gt(userBalance.balance)) {
      return 'Insufficient Collateral'
    }
    if (actionType === 'Withdraw' && BigNumber(amount || 0).gt(userBalance.supplyAmount)) {
      return 'Insufficient Collateral'
    }
    if (actionType === 'Borrow' && BigNumber(amount || 0).gt(userBalance.borrowBalance)) {
      return 'Insufficient Collateral'
    }
    if (actionType === 'Repay' && BigNumber(amount || 0).gt(userBalance.borrowAmount)) {
      return 'Insufficient Collateral'
    }
    return ''
  }, [actionType, amount, userBalance])
  ...
  const primaryClass = useMemo(() => {
    return props.metaInfo?.type === 'supply'
      ? 'bg-$fi text-bg1 hover:bg-$fi-hover'
      : 'bg-$purple text-fg1 hover:bg-$purple-hover'
  }, [props.metaInfo?.type])
